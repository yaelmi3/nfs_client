...
class RawTCPClient(Client):
    ...
    def do_call(self):
        ...
        if xid != self.lastxid:
            ...
            raise RuntimeError (f'wrong xid in reply {xid} instead of {self.lastxid}')
...
class PartialPortMapperClient:
    ...
    def Set(self, mapping):
        return self.make_call(PMAPPROC_SET,
                              mapping,
                              self.packer.pack_mapping,
                              self.unpacker.unpack_uint)

    def Unset(self, mapping):
        return self.make_call(PMAPPROC_UNSET,
                              mapping,
                              self.packer.pack_mapping,
                              self.unpacker.unpack_uint)

    def Getport(self, mapping):
        return self.make_call(PMAPPROC_GETPORT,
                              mapping,
                              self.packer.pack_mapping,
                              self.unpacker.unpack_uint)

    def Dump(self):
        return self.make_call(PMAPPROC_DUMP,
                              None,
                              None,
                              self.unpacker.unpack_pmaplist)

    def Callit(self, ca):
        return self.make_call(PMAPPROC_CALLIT,
                              ca,
