...
class MountPacker(Packer):
    ...
    def pack_fhandle(self, fhandle):
        fh_length = len(fhandle)
        self.pack_uint(fh_length)
        self.pack_fopaque(fh_length, fhandle)
...
class MountUnpacker(Unpacker):
    ...
    def unpack_fhandle(self):
        """
        Using file handle length to unpack fixed length opaque data stream
        """
        fh_length = self.unpack_uint()
        return self.unpack_fopaque(fh_length)

    def unpack_fhstatus(self):
        status = self.unpack_uint()
        if status == 0:
            fh = self.unpack_fhandle()
            [self.unpack_uint() for _ in range(self.unpack_uint())]
        else:
            fh = None
        return status, fh
...
class PartialMountClient:
    ...
    def mount(self, directory):
        return self.make_call(1, directory.encode(),
